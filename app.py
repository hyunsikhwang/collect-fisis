...
if selected_tab == "📈 분석 대시보드 (Dashboard)":
    ...
    if not analysis_df.empty:
        ...
        with left_col:
            ...
            line = Line(init_opts=opts.InitOpts(width="100%", height="540px", theme="white", renderer="svg"))
            ...
            st_pyecharts(line, height="540px", key="dashboard_line_chart", renderer="svg")

        with right_col:
            if company_ts_df.empty:
                st.warning("회사별 시계열 데이터가 없어 회사 차트를 표시할 수 없습니다.")
            else:
                company_line = Line(init_opts=opts.InitOpts(width="100%", height="540px", theme="white", renderer="svg"))
                ...
                st_pyecharts(company_line, height="540px", key="dashboard_company_line_chart", renderer="svg")
